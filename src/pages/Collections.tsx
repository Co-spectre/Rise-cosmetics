...
const Collections = () => {
  ...
  const handleAddToCart = (product: Product) => {
    addToCart({
      id: parseInt(product.id) || Date.now(), // Fallback to timestamp for string IDs
      name: product.name,
                    ...
                    <div className="relative bg-gradient-to-b from-gray-900 to-black border border-gold/20 rounded-2xl overflow-hidden shadow-2xl hover:shadow-gold/20 transition-all duration-500 group-hover:scale-105">
                      ...
                      <div className="p-6">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            <Link to={`/product/${product.id}`}>
                              <h3 className="text-lg font-bold text-white mb-1 hover:text-gold transition-colors cursor-pointer">
                                {product.name}
                              </h3>
                            </Link>
